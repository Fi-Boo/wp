  ...
  foreach ($rowData as $data) {
    ...
    echo <<<"STR"
      <tr id="details-tr-{$data[0]}">
        ...
        <td>
          <input type="{$type}" name="user[{$data[0]}]" value="{$value}" placeholder="{$data[2]}" pattern="{$data[3]}" onclick="removeDetailsError('{$data[0]}')" onchange="localStorageFunc('remember')" required>
